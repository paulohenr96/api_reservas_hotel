...
public class ReservaControllerTest {
	...
	void novaReservaSucesso() throws Exception {
		...
		mockMvc.perform(post("/reservas").contentType(MediaType.APPLICATION_JSON).content(asJsonString(reserva))
				.param("idQuarto", id + "")).andExpect(status().isOk()).andExpect(content().string(""));
//		verify(service).novaReserva(reserva,id );
	...
	void findAllReservasSucesso() throws Exception {
		List<ReservaDTO> lista = List.of(new ReservaDTO(1L), new ReservaDTO(2L), new ReservaDTO(3L));

		Page<ReservaDTO> pagina = new PageImpl<>(lista);

		when(service.getAll(any(PageRequest.class))).thenReturn(pagina);

		mockMvc.perform(get("/reservas").param("size", "3").param("page", "0")).andExpect(status().isOk())
	...
	void findAllReservasPorDataSucesso() throws Exception {
		...
		mockMvc.perform(get("/reservas/pordata/").param("data", data).param("size", "3").param("page", "0"))
	...
	void findAllReservasPorQuartoSucesso() throws Exception {
		...
		mockMvc.perform(get("/reservas/porquarto/" + id).param("size", "3").param("page", "0"))
				.andExpect(status().isOk()).andExpect(jsonPath("$.content").isArray());
	}
	...
	void findByIdReservaSucesso() throws Exception {
		...
		mockMvc.perform(get("/reservas/" + id)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(id));
	}
